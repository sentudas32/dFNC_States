...
cl_path_gm='/Users/sukeshdas/Documents/SKD/ECP_Exp/OutPut_ECP_SmoothSeparate_New/sFC_Cluster_FDR/cl_map_sFC_HC_lrTLE_gm_new.nii';
cl_path_wm='/Users/sukeshdas/Documents/SKD/ECP_Exp/OutPut_ECP_SmoothSeparate_New/sFC_Cluster_FDR_GM_WM_SC/cl_map_sFC_HC_lrTLE_wm_new_k11.nii';
cl_path_sc='/Users/sukeshdas/Documents/SKD/ECP_Exp/OutPut_ECP_SmoothSeparate_New/sFC_Cluster_FDR_GM_WM_SC/cl_map_sFC_HC_lrTLE_sc_new_k6.nii';

dp_hc_tle = '/Users/sukeshdas/Documents/SKD/Data/ECP/ECP_SmoothSeparate_New/func/HC_TLE';
dp_hc='/Users/sukeshdas/Documents/SKD/Data/ECP/ECP_SmoothSeparate_New/func/HC';
dp_ltle='/Users/sukeshdas/Documents/SKD/Data/ECP/ECP_SmoothSeparate_New/func/TLE_Left';
dp_rtle='/Users/sukeshdas/Documents/SKD/Data/ECP/ECP_SmoothSeparate_New/func/TLE_Right';
output_folder = '/Users/sukeshdas/Documents/SKD/ECP_Exp/OutPut_ECP_SmoothSeparate_New/sFC_Cluster_FDR_GM_WM_SC/sFC_dFC_analysis/';
...
fun_file='/Users/sukeshdas/Documents/SKD/Data/ECP/ECP_SmoothSeparate_New/func/HC/sub-EC1006/wsfREG_rc_sub-EC1006_task-rest_dir-AP_run-1_bold.nii';
...
sc_cl_vol=double(niftiread(cl_path_sc));
sc_cl_mat=reshape(sc_cl_vol,nx*ny*nz,1);
cl_sc=unique(sort(sc_cl_mat(:)));
cl_sc=cl_sc(2:end);
noc_sc=length(cl_sc);
for icl=1:1:noc_sc
    cl_mat(find(sc_cl_mat==cl_sc(icl)))=noc_gm+noc_wm+icl;
end
...
dir_hc_tle=dir_hc_tle(~ismember({dir_hc_tle.name},{'.','..','.DS_Store'}));
nos1=length(dir_hc_tle); % No of initial subjects
dir_hc=dir(dp_hc);
dir_hc=dir_hc(~ismember({dir_hc.name},{'.','..','.DS_Store'}));
dir_ltle=dir(dp_ltle);
dir_ltle=dir_ltle(~ismember({dir_ltle.name},{'.','..','.DS_Store'}));
dir_rtle=dir(dp_rtle);
dir_rtle=dir_rtle(~ismember({dir_rtle.name},{'.','..','.DS_Store'}));
...
dFCv_stat_ltle=dFCv_stat(find(Gr==-1),:);
...
if(State_Analysis==1)
    ...
    dm='correlation';%'sqeuclidean';% 'cosine','correlation'
    num_replicates=100;%50;
    ...
    title('Elbow Method')
    saveas(gcf,fullfile(output_folder,'K_elbow_dFNC_GM_WM_SC_new.png'))
    ...
    close all
    K=7;
    gr_plt=[-1 1];  % 0 for HC, 1 for rTLE, -1 for lTLE
    gr_plt_str={'lTLE','rTLE'};
    % [id,cl,seed] = kmeans(dFCv_stat,K,'distance',dm,'replicates',num_replicates);            % K-means clustering
    % save([output_folder,'ROI_CL_dFNC_W35_sl5_st7_id_dcorr1'],'id');
    % save([output_folder,'ROI_CL_dFNC_W35_sl5_st7_centers_dcorr1'],'cl');

    load([output_folder,'ROI_CL_dFNC_W35_sl5_st7_id_dcorr1']);
    load([output_folder,'ROI_CL_dFNC_W35_sl5_st7_centers_dcorr1']);
    ...
    lbls_sc="SC" +string(1:noc_sc);
    lbls=[lbls_gm,lbls_wm,lbls_sc];
    ...
    emptyTiles = totalTiles - K;
    % How many empty tiles go in last row
    shift = floor(emptyTiles/2);
    lastRowStart = nCols*(nRows-1) + 1;
    usedTiles = [];
    state_str={'(a) State 1','(b) State 2','(c) State 3','(d) State 4','(e) State 5','(f) State 6','(g) State 7','(h) State 8'};
    ...
    for ik = 1:K
        ...
        usedTiles(end+1) = tileIndex;
        nexttile(tileIndex)

        h=heatmap(states(:,:,ik),'ColorLimits',[min(states(:)) max(states(:))]);
        title(state_str{ik})
        colorbar;
        colormap(jet)
        ylabel('FNs');
    ...
    unused = setdiff(1:totalTiles, usedTiles);
    for ik = unused
        nexttile(ik)
        axis off
    end
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    cen_sub_states_hc=cen_sub_states(:,find(Sub_G==gr_plt(1)),:);
    cen_sub_states_tle=cen_sub_states(:,find(Sub_G==gr_plt(2)),:);
    ...
    nCols = 4;
    % Number of full column pairs per block
    nRowPairs = ceil(K / nCols);
    nRows = 2 * nRowPairs;  % Each block = HC row + TLE row

    figure('Units','normalized','Position',[0.05 0.05 0.9 0.85])
    t = tiledlayout(nRows, nCols, 'TileSpacing','tight', 'Padding','tight');

    letters = 'abcdefghijklmnopqrstuvwxyz';
    plotIndex = 1;   % For lettering

    for block = 1:nRowPairs
        % States in this block
        stateStart = (block-1)*nCols + 1;
        stateEnd = min(block*nCols, K);

        nThisBlock = stateEnd - stateStart + 1;
        for c = 1:nThisBlock
            stateIdx = stateStart + (c-1);
            % Tile position = row 1 of this block + column
            nexttile( (block-1)*2*nCols + c )

            h = heatmap(hc_dFNC_states(:,:,stateIdx), 'ColorLimits',[min_level max_level]);
            colormap(jet); colorbar
            h.XDisplayLabels = lbls;
            h.YDisplayLabels = lbls;
            h.FontSize = 8;
            title(['(' letters(plotIndex) ') State ' num2str(stateIdx) ' ' gr_plt_str{1}])
            plotIndex = plotIndex + 1;
        end
        for c = 1:nThisBlock
            stateIdx = stateStart + (c-1);
            % Tile position = next row in same column
            nexttile( (block-1)*2*nCols + nCols + c )

            h = heatmap(tle_dFNC_states(:,:,stateIdx), 'ColorLimits',[min_level max_level]);
            colormap(jet); colorbar
            h.XDisplayLabels = lbls;
            h.YDisplayLabels = lbls;
            h.FontSize = 8;
            title(['(' letters(plotIndex) ') State ' num2str(stateIdx) ' ' gr_plt_str{2}])
            plotIndex = plotIndex + 1;
        end
    end
    ...
    for ik=1:1:K
        ...
        title(strcat('(a) State ',num2str(ik), ', dFNC (',gr_plt_str{1},')'))
        h1.FontSize = 8;
        ...
        title(strcat('(b) State ',num2str(ik), ', dFNC (',gr_plt_str{2},')'))
        h2.FontSize = 8;
